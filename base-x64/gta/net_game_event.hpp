...
namespace rage
{
	...
	public:
		...
		{
			uint32_t read;
			if (base::g_Game->m_read_datbitbuffer_dword(this, &read, bits))
			{
				*integer = read;
				return true;
			}
		...
		{
			return base::g_Game->m_read_datbitbuffer_dword(this, integer, bits);
		}
		...
		inline bool WriteInteger(std::uint32_t integer, int bits)
		{
			return base::g_Game->m_write_integer_data_writer(this, integer, bits);
		}
