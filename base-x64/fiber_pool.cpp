...
namespace base
{
	...
	void base::fiber_pool::queue_thread(std::function<void()> func)
	{
		if (func)
		{
			std::lock_guard lock(m_mutex2);
			m_threadjobs.push(std::move(func));
		}
	}
	...
	void base::fiber_pool::thread_tick()
	{
		std::unique_lock lock(m_mutex2);
		if (!m_threadjobs.empty())
		{
			auto job = std::move(m_threadjobs.top());
			m_threadjobs.pop();
			lock.unlock();
